 ...
 */
class PasswordLockDeviceConfigActivity: AppCompatActivity() {

    companion object {
        fun startActivity(context: Context?, devId: String?) {
            val intent = Intent(context, PasswordLockDeviceConfigActivity::class.java)
            //设备id
    ...
    private fun getLockDeviceConfig() {
        tuyaLockDevice!!.getLockDeviceConfig(object : IThingResultCallback<String?> {
            override fun onSuccess(result: String?) {
                Log.i(
                    Constant.TAG,
                    "getLockDeviceConfig success :" + JSONObject.toJSONString(result)
                )
            ...
            override fun onError(errorCode: String, errorMessage: String) {
                Log.e(
                    Constant.TAG,
                    "getLockDeviceConfig failed: code = $errorCode  message = $errorMessage"
                )
                password_content!!.text = errorMessage
            }
        })
    }
