 ...
 */
class PasswordProOnlineDetailActivity : AppCompatActivity(),
    ...
    private var mPasswordData: ProTempPasswordItem? = null

    private var passwordValue: String? = null

    companion object {
        fun startActivity(
            context: Context, passwordItem: ProTempPasswordItem?,
            devId: String?, from: Int
        ) {
            val intent = Intent(
                context,
                PasswordProOnlineDetailActivity::class.java
            )
            //设备id
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mPasswordData = JSONObject.parseObject(
            ...
            ProTempPasswordItem::class.java
        )
        if (null == mPasswordData) {
            mPasswordData = ProTempPasswordItem()
        } else {
    ...
    private fun createPassword() {
        ...
        if (mFrom == 1) {
            passwordRequest.id = mPasswordData!!.unlockBindingId
        }
        Log.i(Constant.TAG, "request:$passwordRequest")
        if (mFrom == 0) {
            tuyaLockDevice!!.getProCustomOnlinePassword(
                passwordRequest,
            ...
            tuyaLockDevice!!.updateProOnlinePassword(
                passwordRequest,
                object : IThingResultCallback<String?> {
                    override fun onSuccess(result: String?) {
                        Toast.makeText(applicationContext, "onSuccess", Toast.LENGTH_SHORT).show()
