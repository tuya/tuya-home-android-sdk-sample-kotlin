...
/**
 *
 * Created by nielev on 2024/6/6
 */
class GoogleVoiceSettingActivity: AppCompatActivity() {
    ...
    companion object{
        fun startActivity(context: Context, devId: String?) {
            val intent = Intent(context, GoogleVoiceSettingActivity::class.java)
            //设备id
    ...
    private fun isRemoteUnlockAvailable() {
        wifiLock!!.fetchRemoteVoiceUnlock(object : IThingResultCallback<Boolean> {
    ...
    private fun setVoicePassword() {
        wifiLock!!.setRemoteVoiceUnlock(isOpen, password, object : IThingResultCallback<Boolean> {
            override fun onSuccess(result: Boolean) {
                if (result) {
