...
class VoiceSettingActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        set_remote_group.setOnCheckedChangeListener { _: RadioGroup?, checkedId: Int ->
    ...
    private fun isRemoteUnlockAvailable() {
        tuyaLockDevice!!.fetchRemoteVoiceUnlock(object : IThingResultCallback<Boolean> {
            override fun onSuccess(result: Boolean) {
                Log.i(
                    Constant.TAG,
                    "get remote unlock available success:$result"
                )
    ...
    private fun setVoicePassword() {
        tuyaLockDevice!!.setRemoteVoiceUnlock(
            isOpen,
            password,
            object : IThingResultCallback<Boolean> {
                override fun onSuccess(result: Boolean) {
                    if (result) {
                        Toast.makeText(applicationContext, "setting success", Toast.LENGTH_SHORT)
                            .show()
                        remote_unlock_available!!.text = isOpen.toString()
                    } else {
                        Toast.makeText(applicationContext, "setting fail", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
